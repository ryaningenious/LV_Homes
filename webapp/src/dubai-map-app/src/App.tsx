...
function App() {
  ...
  return (
    <div className="relative">
      <div
        id="fixed-nav"
        className="fixed top-0 left-0 z-90 right-0 duration-300 ease-in-out bg-gradient-to-b from-black from-[-10%] to-transparent"
      >
        <div className="max-w-7xl mx-auto flex justify-between items-center px-4 md:px-6">
          ...
          <div
            id="nav-menus"
            className="flex-1 hidden md:flex items-center gap-4 md:gap-8 justify-center pr-4 bg-gradient-to-r from-gold-100 via-gold-200 via-gold-400 via-gold-300 to-gold-100 bg-clip-text animate-shining font-montserrat py-2"
          >
            {menuItems.map((menuItem, index) => (
              <a
                key={index}
                className={`menu hover:text-transparent hover:font-semibold ${menuItem.cssClassName}`}
                href={menuItem.url}
              >
                {menuItem.label}
              </a>
